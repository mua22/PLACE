...
/**
 *  FactLink: Class to represent a fact, and links with actions
 *  which link to that fact (has a fact as its precond or effects)
 */
public class FactLink {
    ...
    /* Function to selectively add random cost at time point t and cost c but not
       at the time point that is higher than all others considered */
	...
	if( index < 1 ) {
	    System.out.println("Error in FactLink.getCost(): cost = infinite -- factID = "
			       + factID);
	    System.exit(1);
	}
	...
	if( time < time_point[0] ) {
	    System.out.println("Error in FactLink.getCost(): 0 < time = " + time
			       + " < time_point[0] = " + time_point[0] +
			       " -- factID = " + factID);
	    System.exit(1);
	}
	...
	// *time* is smaller than the first update
	if( index < 1 ) {
	    System.out.println("Error in FactLink.getCostAction() - cost = infinite. FactID = "
			       + factID);
	    System.exit(1);
	}
	...
	if( time < time_point[0] ) {
	    System.out.println("Error in FactLink.getCostAction(): 0 < time < time_point[0] "
			       + " time = " + time + " time_point[0] = " + time_point[0]
			       + " cost[0] = " + cost[0] + " -- FactID = " + factID);
	    System.exit(1);
	}
    ...
    /**
     * BM: Added July 23, 2003 to support "at end" condition
     * This function check if this fact is achievable at a given time
     */
