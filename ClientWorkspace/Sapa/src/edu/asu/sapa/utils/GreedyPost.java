...
/**
 *   GreedyPost.java: Greedy Post Processing the solution to
 *   greedily build in linear time the correspondent p.o plan. The
 *   topological sort of that p.o plan would give the possible
 *   better makespan plan.
 */
public class GreedyPost {
    ...
    /**
     * Function to initialize the datastructures in the GreedyPost class
     * to build the o.c. plan greedily later.
     */
    public void initialize(ArrayList actions, ArrayList actionSigList,
			   ArrayList times, ArrayList durs,
			   float tdur, float bms) {
	int i;
	GAction anAction;
    ...
    /**
     * Function to find a list of actions that support a given *prop* at
     * time point earlier than preTime (candidate set)
     */
    ...
    /**
     * Function to set up the orderings based on the mutex relation between
     * two actions. Note that we already have: actIndex1 < actIndex2.

     * This function assume the SHORTCUT that all precondition start from
     * the starting time point, and the delete effect assumedly start
     * taking effect from the starting point also.
     *
     * Note: Can be changed for better separating duration calculation
     * according to the different between PDDL2.1 mutex rules and the
     * original mutex rules in Sapa.
     */
    ...
    /**
     * Function to set the mutex ordering between two actions because they
     * use the same resource. Note that actIndex2 start "after" actIndex1.
     */
    ...
    /**
     * Functions to take the set of grounded actions that represent
     * the fixed-time plan returned by Sapa, and their fixed execution
     * time and durations and greedily build the causal structure
     * from it.
     *
	...
	System.out.println(";; POP: CausalLink = " + numCL + " | LogicalMutex = "
					   + numLM + " | ResourceMutex = " + numRM);
	...
	for(i = 0; i < planSize; i++)
		...
		anOCPlan.addLogOrder(oRel.relation,oRel.actIndex,i,oRel.saTime==0,oRel.eaTime==0,oRel.relID,0);
	    }
...
//	outputPlan = "";
	...
	System.out.println(";; Actions: " + (planSize - 2) + " Makespan: " + makespan
			   + "    . Sum dur: " + totalDur);
	output += "<p>Makespan:         " + makespan
			+ "<p>Sum duration:         " + totalDur;

	// Print the final plan if it's better than the best plan (in term of Makespan) found so far
     ...
     * if it's implied by transitive closure. 
     * This function should be called only AFTER calling function buildOrdering().
     */
